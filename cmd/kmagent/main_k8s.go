...
func main() {
	...
	defer func() {
		...
		if syncErr := agent.Logger.Sync(); syncErr != nil && syncErr.Error() != "sync /dev/stdout: invalid argument" {
			// Ignore "invalid argument" error for stdout/stderr in some environments
...
func handleSignals(cancelFunc context.CancelFunc) {
	...
	go func() {
		sig := <-sigChan
		log.Printf("Received signal %s, initiating shutdown...", sig) // Use log for simple signal handling
